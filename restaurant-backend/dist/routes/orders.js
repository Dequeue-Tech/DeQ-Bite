...
router.post('/:id/items', restaurant_1.requireRestaurant, async (req, res) => {
    try {
        ...
        const existingPaidAmount = existingOrder.paidAmountPaise ?? 0;
        ...
        const updatedPaymentStatus = updatedDue === 0 ? 'COMPLETED' : existingPaidAmount > 0 ? 'PARTIALLY_PAID' : 'PENDING';
        const [, updatedOrder] = await database_1.prisma.$transaction([
            database_1.prisma.orderItem.createMany({
...
router.get('/restaurant/all', restaurant_1.requireRestaurant, (0, restaurant_1.authorizeRestaurantRole)('OWNER', 'ADMIN', 'STAFF'), async (req, res) => {
    try {
        ...
        const whereWithDelta = hasValidUpdatedAfter
            ? {
                ...where,
