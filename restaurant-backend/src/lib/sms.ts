...
type SMSProviderName = "fast2sms" | "twilio" | "disabled";
/**
...
const getConfiguredSMSProvider = (): SMSProviderName => {
  ...
  logger.warn("Unsupported SMS_PROVIDER, defaulting to fast2sms", {
    configured,
  });
  return "fast2sms";
};
...
const sendViaFast2SMS = async (options: SMSOptions): Promise<boolean> => {
  ...
  if (!number) {
    logger.error("Fast2SMS requires a valid 10-digit Indian mobile number", {
      to: options.to,
    });
    return false;
  }
...
export async function sendSMS(options: SMSOptions): Promise<boolean> {
  ...
  if (!isValidE164(normalizedTo)) {
    logger.error("Invalid destination phone number", {
      to: options.to,
      normalizedTo,
    });
    return false;
