...
const buildInsights = (snapshot: MetricSnapshot, periodType: AnalyticsPeriodType, periodStart: Date, periodEnd: Date) => {
...
export const generateAnalyticsSnapshot = async (params: {
  restaurantId: string;
  ...
  const saved = await prisma.analyticsSnapshot.upsert({
    where: {
